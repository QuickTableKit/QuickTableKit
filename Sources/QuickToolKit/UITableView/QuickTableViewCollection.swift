...
extension QuickTableViewCollection {
    ...
    public func index<SectionId: RawRepresentable>(sectionWithId sectionId: SectionId) -> Int? where SectionId.RawValue == Int {
        sections.firstIndex(where: { $0.id == sectionId.rawValue })
    }
    
    public func index(cellWithId id: String) -> IndexPath? {
        for (sectionIndex, section) in sections.enumerated() {
            guard let itemIndex = section.index(cellWithId: id) else {
                continue
            }
    ...
    public func index<SectionId: RawRepresentable, CellId: RawRepresentable>(cellWithSectionId sectionId: SectionId, cellId: CellId) -> IndexPath? where SectionId.RawValue == Int, CellId.RawValue == Int {
