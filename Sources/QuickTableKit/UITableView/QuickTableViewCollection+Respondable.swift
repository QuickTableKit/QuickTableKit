...
extension QuickTableViewCollection {
    ...
    public func configureRespondableCells(in tableView: UITableView) {
        ...
        for (sectionIndex, section) in sections.enumerated() {
            for (itemIndex, item) in section.items.enumerated() {
                guard item is QuickRespondableTableViewCellModelProtocol else {
                    continue
                }
    ...
    private func updateCell(indexPath: IndexPath, isPreviousInputEnabled: Bool, isNextInputEnabled: Bool, _ tableView: UITableView) {
        guard let cellModel = cell(at: indexPath) as? QuickRespondableTableViewCellModelProtocol else {
            return
        }
        
        let config = QuickRespondableConfig(
